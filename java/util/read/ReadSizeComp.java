...
public class ReadSizeComp {
	private static String path = "C:\\Users\\ksemer\\Desktop\\competitor\\o2\\";
	private static String dataset = "synthetic_pr_0.9_s__";
		...
		for (int m = 1; m <= 4; m++) {
			...
			for (int k = 2; k <= 8; k += 2) {
				for (int it = 0; it < ITERATION_DATA; it++) {
					...
					if (k == 2) {
						// res = read(m, 3, path + dataset + "3_it_" + it +
						// "_m=" + m + "_k=3" + extra);
						// result[m-1][j+1]+= res;
					}
				}
			...
			for (int k = 2; k <= 8; k += 2) {
				...
				for (int IT = 0; IT < ITERATION_DATA; IT++) {
					...
					for (int it = 1; it <= 5; it++) {
						...
						if (k == 2) {
							// res_ = read(m, 3, path + "random/" + dataset +
							// "3_it_" + IT + "_m=" + m + "_k=3" + ex);
				...
				if (k == 2) {
					// out+=("(" + "30," +
					// df.format(results[m-1][j+1]/(ITERATION_DATA * 5)) + ")");
